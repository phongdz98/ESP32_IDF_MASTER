...
bool app_nvs_load_sta_creds(void)
{
    ...
    if (nvs_open(app_nvs_sta_creds_namespace, NVS_READONLY, &handle) == ESP_OK)
    {
        ...
        {
            free(wifi_config_buff);
            printf("app_nvs_load_sta_creds:  Error (%s) no password wifi found in NVS\n", esp_err_to_name(esp_err));
        }
...
esp_err_t app_nvs_clear_sta_creds(void)
{
    nvs_handle handle;
    esp_err_t esp_err;
    ESP_LOGI(TAG, "app_nvs_clear_sta_creds: Clearing WiFi station credentials form flash");
    ...
    if (esp_err != ESP_OK)
    {
        printf("app_nvs_erase_creds: Error (%s) erasing station mode credentials\n", esp_err_to_name(esp_err));
        return esp_err;
    }
    ...
    if (esp_err != ESP_OK)
    {
        printf("app_nvs_erase_creds: Error (%s) NVS commit\n", esp_err_to_name(esp_err));
        return esp_err;
    }

    nvs_close(handle);

    printf("app_nvs_erase_creds: returned ESP_OK\n");
    return ESP_OK;
};