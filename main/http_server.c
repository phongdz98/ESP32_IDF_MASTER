...
/**
 * ESP32 timer configuration passed to esp_timer_created
 */
const esp_timer_create_args_t fw_update_reset_args = {
    .callback = http_fw_update_reset_callback,
    .arg = NULL,
...
static void httpd_server_fw_update_reset_timer(void)
{
    ...
    {
        ESP_LOGI(TAG, "httpd_server_fw_update_reset_timer: FW updated successful staring FW update reset timer ");
    ...
    {
        ESP_LOGI(TAG, "httpd_server_fw_update_reset_timer: FW update unsuccessful");
    }
}
...
/**
 * Jquery get handler is  requested when acessing the web page
...
/**
 * app.css get handler is requested when acessing the web page
 * @param req HTTP request for which the uri needs to be handled
 * @return ESP_OK
 */
...
/**
 * app.js get handler is requested when acessing the web page
 * @param req HTTP request for which the uri needs to be handled
 * @return ESP_OK
 */
...
/**
 * Send the icon file when acessing the web page
 * @param req HTTP request for which the uri needs to be handled
 * @return ESP_OK
 */
 ...
 * @param req HTTP request for which th uri needs to be handled
...
esp_err_t http_server_OTA_update_handler(httpd_req_t *req)
{
    ...
    do
    {
        ...
        printf("httpd_server_OTA_update_handler: OTA RX: %d of %d\r", content_received, content_length);
        ...
        if (!is_req_body_started)
        {
            ...
            if (err != ESP_OK)
            {
                printf("httpd_server_OTA_update_handler: Error with OTA begin, cancelling OTA\r\n");
                return ESP_FAIL;
            }
            else
            {
                printf("httpd_server_OTA_update_handler: Writing to partition subtype 0x%x at offset0x%x\r\n", update_partition->subtype, update_partition->address);
            }
    ...
    if (esp_ota_end(ota_handle) == ESP_OK)
    {
        if (esp_ota_set_boot_partition(update_partition) == ESP_OK)
        {
            ...
            ESP_LOGI(TAG, "http_server_OTA_update_handler: Next boot partition subtype 0x%x at offset 0x%x", boot_partition->subtype, boot_partition->address);
...
/**
 * OTA status handler responds with  the fimware update status after the OTA update is started
...
/**
 * DHT sensor reading JSON handler responds with DHT11 sensor data
 * @param req HTTP request for which th uri needs to be handled
 * @return ESP_OK
 */
static esp_err_t http_server_get_dht_sensor_json_handler(httpd_req_t *req)
{
    ESP_LOGI(TAG, "/dhtSensor.json requested");
    char dhtSensorJON[100];

    // printf("json response: %s", dhtSensorJON);
    if (dht11_cur_data.status == DHT11_OK)
    {
        sprintf(dhtSensorJON, "{\"temp\":%d, \"humidity\":%d}", dht11_cur_data.temperature, dht11_cur_data.humidity);
        httpd_resp_set_type(req, "application/json");
        httpd_resp_send(req, dhtSensorJON, strlen(dhtSensorJON));
        ESP_LOGI(TAG, "/dhtSensor.json read data successfully");
    }
    else
    {
        ESP_LOGI(TAG, "/dhtSensor.json error  dht status %d", dht11_cur_data.status);
    }
...
/**
 * wifiDisconnect.json handler responds by seding a message to the WiFi application to disconnect.
 * @param req HTTP request for which the uri needs to be handled.
 * @return ESP_OK
 */
...
static httpd_handle_t http_server_configure(void)
{
    ...
    xTaskCreatePinnedToCore(&http_server_monitor, "http_server_monitor", HTTP_SERVER_TASK_STACK_SIZE, NULL, HTTP_SERVER_MONITOR_PRIORITY, &task_http_server_monitor, HTTP_SERVER_MONITOR_CORE_ID);

    // Crate the message queue
    ...
    if (httpd_start(&http_server_handle, &config) == ESP_OK)
    {
        ...
        httpd_uri_t dht_sensor_json = {
            .uri = "/dhtSensor.json",
            .method = HTTP_GET,
            .handler = http_server_get_dht_sensor_json_handler,
            .user_ctx = NULL};
        ...
        // Register wifiConnectStatus handler
...
{
    ESP_LOGI(TAG, "http_fw_update_reset_callback: Timer timed out, restarting the device");
    esp_restart();
};
